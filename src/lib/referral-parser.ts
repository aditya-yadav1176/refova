...
const CATEGORY_KEYWORDS: Record<CategorySlug, string[]> = {
  ...
  shopping:      ["shop", "fashion", "clothes", "order", "cart", "checkout", "discount", "sale", "purchase", "buy", "mall", "apparel", "delivery"],
  food:          ["food", "eat", "delivery", "restaurant", "meal", "grocery", "kitchen", "snack", "lunch", "dinner", "order", "cuisine"],
...
function extractCode(text: string): string | null {
  ...
  if (dashToken) return dashToken[1].toUpperCase();
  ...
  if (caps && caps[1] !== caps[1].toLowerCase()) return caps[1];
...
function extractAmount(text: string): { value: string; type: BenefitType } | null {
  ...
  // Free month / trial
  if (/free\s+(?:month|trial|year|week)/i.test(text)) return { value: "FREE TRIAL", type: "free-month" };
  // Free credits / reward
...
function buildSummary(service: string, benefit: string, code: string, category: CategorySlug): string {
  const labels: Record<CategorySlug, string> = {
    finance: "finance app", shopping: "shopping platform", food: "food delivery app",
    travel: "travel platform", education: "learning platform", productivity: "productivity tool",
    developer: "developer platform", entertainment: "entertainment platform",
  };
  const action =
    benefit.toLowerCase().includes("cashback") ? "cashback" :
    benefit.toLowerCase().includes("% off") ? "discount" :
    benefit.toLowerCase().includes("free") ? "free access" : "reward";
...
function buildTags(service: string, category: CategorySlug, benefit: string): string[] {
  ...
  const catTag: Record<CategorySlug, string> = {
    finance: "fintech", shopping: "shopping", food: "food", travel: "travel",
    education: "learning", productivity: "productivity", developer: "devtools", entertainment: "entertainment",
  };
  tags.push(catTag[category]);
...
function mockParse(raw: string): ParsedReferral {
  ...
  return { service, initials, code, isLink, benefit, benefitType, category, summary, conditions, expires, tags };
}
